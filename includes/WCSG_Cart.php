<?php
	class WCSG_Cart {
		...
		public static function init() {
			...
			add_filter( 'woocommerce_update_cart_action_cart_updated', __CLASS__ . '::cart_update' );
		...
		public static function add_gifting_option_cart( $title, $cart_item, $cart_item_key ) {
			...
			if ( is_cart() && WC_Subscriptions_Product::is_subscription( $cart_item['data'] ) ) {
				if ( !isset( $cart_item['wcsg_gift_recipients_email'] ) ) {
					echo WCS_Gifting::generate_gifting_html( $cart_item_key, '' );
				} else {
					echo WCS_Gifting::generate_gifting_html( $cart_item_key, $cart_item['wcsg_gift_recipients_email'] );
				}
			}
		}
