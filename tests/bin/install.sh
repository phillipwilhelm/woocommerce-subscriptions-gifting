...
install_cs() {
	...
	curl -L https://api.github.com/repos/squizlabs/PHP_CodeSniffer/tarball/?access_token=$GITHUB_TOKEN | tar --strip-components=1 -zx -C "php-codesniffer"
	...
	curl -L https://api.github.com/repos/WordPress-Coding-Standards/WordPress-Coding-Standards/tarball/?access_token=$GITHUB_TOKEN | tar --strip-components=1 -zx -C "wordpress-coding-standards"

	# make a directory for the Prospress coding standard rules
	mkdir -p "prospress-coding-standards"

	# uncompress the coding standards into the directory we created
	curl -L https://api.github.com/repos/Prospress/prospress-coding-standards/tarball/?access_token=$GITHUB_TOKEN | tar --strip-components=1 -zx -C "prospress-coding-standards"
